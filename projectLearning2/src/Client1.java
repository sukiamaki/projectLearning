...
public class Client1 extends JFrame implements MouseListener{
	...
	private boolean run_fin = false;	//待ち状態を抜ける

	// コンストラクタ
	public Client1(Othello game, Player player) {
	...
	public void receiveData(String msg) {
		...
		if(length>1) {	//少なくとも1つはひっくり返された時(=パスではない時)
	...
	///////////////////*データの送信*//////////////////////
	public void sendData(String changeStone) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		System.out.println(command + "操作を受け付け");
	...
	public void acceptOperation(String command){	
		...
		if(command.equals("pass")) {		
			game.setTurn(game.changeColor(game.getTurn()));	//手番変更
			turnLabel_draw();
			sendData(","); //操作情報の送信
		}
		//投了を選択した時
		else if(command.equals("stop")) {
			game.changeGiveUp("me");
			sendMessage("3投了");
		}
		//石を置こうとした時
		else {	
			if(game.getTurn().equals(player.getColor())) {	//自分のターンであれば
				...
				if(game.putStone(theButton_num,game.getTurn(), true) == true) {	//石が置けるかの確認と盤面情報の変更
					...
					turnLabel_draw();	//画面に描画
					sendData(game.getChangeStone()); //操作情報の送信
				}
				
			}
		}
		
		run_fin = true;//待ち状態を抜ける
	}
	...
	public void winner_draw() {
		turnLabel.setText("結果は" + game.checkWinner(player.getColor()) + "です");
	}
	...
	public static void main(String args[]){ 
		...
		Client1 oclient = new Client1(game, player);
		...
		while(game.isGameover() == false) {	//終了状態の確認
			Client1.waiting w = oclient.new waiting();

			//自分の手番の時
			if(game.getTurn().equals(player.getColor())) {	
				
				//石を置ける時
				if(game.checkBoard(game.getTurn()) == true){
					w.start();
					System.out.println("自分の操作待ちに入りました");
					try {
						w.join();
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
					System.out.println("自分の操作待ちを抜けました");		//操作の受け入れ作業からデータ送信まで
				}	
				//石が置けない時
				else {
					oclient.pass.setEnabled(true);
					w.start();
					System.out.println("自分の操作待ちに入りました");
					try {
						w.join();
			...
			//相手の手番の時
				...
				w.start();
				System.out.println("相手の操作待ちに入りました");
				try {
					w.join();
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				System.out.println("相手の操作待ちを抜けました");
